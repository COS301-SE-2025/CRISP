...
Creates test data with parallel processing and progress bars.
"""
...
class DatabasePopulator:
    ...
    def create_users(self):
        ...
        if len(self.users) < total_estimated_users * 0.8:  # If we created less than 80% of target
            print(f"Warning: Only created {len(self.users)}/{total_estimated_users} users. Check for errors above.")
    ...
    def create_trust_groups(self):
        ...
        print(f"Creating {self.NUM_TRUST_GROUPS} trust groups with {self.MAX_WORKERS} workers...")
    ...
    def print_summary(self):
        ...
        print(f"  Trust Groups: {len(self.trust_groups)}")

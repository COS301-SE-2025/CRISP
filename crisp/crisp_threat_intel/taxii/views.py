...
class TAXIIBaseView(APIView):
    ...
    def get_trust_level(self, source_org, target_org):
        ...
        from crisp_threat_intel.utils import get_trust_level
        return get_trust_level(source_org, target_org)
    
    def apply_anonymization(self, stix_object, requesting_org, source_org):
        """
        Apply appropriate anonymization based on trust level using advanced anonymization system.
        """
        ...
        try:
            # Import here to avoid circular imports
            from crisp_threat_intel.utils import get_anonymization_level
            
            # Get anonymization level based on trust relationship
            anonymization_level = get_anonymization_level(source_org, requesting_org)
            
            # Use the advanced anonymization context
            context = AnonymizationContext()
            anonymized_json = context.anonymize_stix_object(stix_object, anonymization_level)
            
            # Parse back to dictionary since the method returns JSON string
            anonymized = json.loads(anonymized_json)
            
            # Add anonymization metadata
            anonymized['x_crisp_anonymized'] = True
            anonymized['x_crisp_anonymization_level'] = anonymization_level.value
            anonymized['x_crisp_source_org'] = source_org.name
            anonymized['x_crisp_trust_level'] = self.get_trust_level(source_org, requesting_org)
            
            return anonymized
            
        except AnonymizationError as e:
            # Log error and return None to exclude object from results
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Anonymization failed for STIX object: {e}")
            return None
        except Exception as e:
            # Log unexpected errors and fall back to excluding object
            import logging
            logger = logging.getLogger(__name__)

...
case $TEST_TYPE in
    unit)
        CMD="$CMD core/tests/unit/"
        ;;
    integration)
        CMD="$CMD core/tests/integration/"
        ;;
    api)
        CMD="$CMD core/tests/api/"
        ;;
    all)
        CMD="$CMD core/tests/"
        ;;
...
if [ "$COVERAGE" = true ]; then
    CMD="python3 -m pytest --cov=core.trust --cov-report=term-missing --cov-report=html --cov-report=xml"
    case $TEST_TYPE in
        unit)
            CMD="$CMD core/tests/unit/"
            ;;
...
# Coverage reports are generated automatically by pytest-cov
if [ "$COVERAGE" = true ]; then
    echo "----------------------------------------"

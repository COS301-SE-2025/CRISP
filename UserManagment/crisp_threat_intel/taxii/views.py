...
from crisp_threat_intel.models import Collection, STIXObject, CollectionObject
from crisp_threat_intel.strategies.anonymization import AnonymizationStrategyFactory
from UserManagement.models import Organization
from crisp_project.services import CRISPIntegrationService, TAXIIIntegrationService
...
class TAXIIBaseView(APIView):
    ...
    def get_organization(self, request):
        ...
        """
        return CRISPIntegrationService.get_user_organization(request.user)
...
class CollectionObjectsView(TAXIIBaseView):
    ...
    def get(self, request, collection_id):
        """
        Get objects from a collection with filtering and pagination using trust-based access control.
        """
        ...
        try:
            ...
            # Use trust-based access control instead of simple ownership check
            allowed, access_level, trust_info = CRISPIntegrationService.check_access_permission(
                request.user, collection.owner, 'read'
            )
            
            if not allowed or not collection.can_read:
                return Response(
                    {"error": "Access denied to this collection", "details": trust_info}, 
                    status=status.HTTP_403_FORBIDDEN
                )
            
            # Use integration service to get objects with trust-based anonymization
            limit = int(request.query_params.get('limit', 100))
            added_after = request.query_params.get('added_after')
            
            result = TAXIIIntegrationService.get_collection_objects_with_trust(
                collection_id, 
                request.user, 
                limit=limit, 
            ...
            return JsonResponse(result, content_type=settings.TAXII_SETTINGS.get('MEDIA_TYPE_STIX', 'application/stix+json;version=2.1'))

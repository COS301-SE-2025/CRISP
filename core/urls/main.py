...
from core.views.api import test_runner_views
...
def api_root(request):
    ...
    return Response({
        ...
        'endpoints': {
            ...
            'testing': {
                'send_alert_email': '/api/tests/send-alert-email/ (POST)',
                'run_alert_tests': '/api/tests/run-alert-tests/ (POST)',
                'get_test_counts': '/api/tests/test-counts/ (GET)',
            }
        },
...
# Test Runner URLs
test_urlpatterns = [
    path('send-alert-email/', test_runner_views.send_alert_email, name='send_alert_email'),
    path('run-alert-tests/', test_runner_views.run_alert_tests, name='run_alert_tests'),
    path('test-counts/', test_runner_views.get_test_counts, name='get_test_counts'),
]

# Public API URLs for users
public_user_urlpatterns = [
    path('', user_views.UserListView.as_view(), name='user_list'),
    path('<uuid:pk>/', user_views.UserDetailView.as_view(), name='user_detail'),
]
...
urlpatterns = [
    ...
    path('users/', include(public_user_urlpatterns)),
    path('tests/', include(test_urlpatterns)),
]
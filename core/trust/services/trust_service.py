...
class TrustService:
    ...
    def get_trust_relationships(self, requesting_user, organization_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not org_id:
                # Always return empty list for users with no org, not an error
                if hasattr(requesting_user, 'organization') and getattr(requesting_user, 'organization', None):
                    org_id = str(requesting_user.organization.id)
                else:
                    return {"success": True, "relationships": []}
            # Check access permissions
            if not self._check_organization_access(requesting_user, org_id):
                # For users with no org, always return empty list, not error
                if not hasattr(requesting_user, 'organization') or not getattr(requesting_user, 'organization', None):
                    return {"success": True, "relationships": []}
    ...
    def get_trust_metrics(self, requesting_user, organization_id: str = None) -> Dict[str, Any]:
        """Get trust metrics for an organization"""
        try:
            if not organization_id:
                if hasattr(requesting_user, 'organization') and getattr(requesting_user, 'organization', None):
                    organization_id = str(requesting_user.organization.id)
                else:
                    # Always return zeroed metrics for users with no org, not an error
            ...
            if not self._check_organization_access(requesting_user, organization_id):
                # For users with no org, always return zeroed metrics, not error
                if not hasattr(requesting_user, 'organization') or not getattr(requesting_user, 'organization', None):
                    return {"success": True, "metrics": {
                        "trust_score": 0,
                        "total_relationships": 0,
    ...
    def get_trust_groups(self, requesting_user, organization_id: str = None) -> Dict[str, Any]:
        """Get trust groups for a user's organization"""
        try:
            org_id = organization_id
            if not org_id:
                if hasattr(requesting_user, 'organization') and getattr(requesting_user, 'organization', None):
            ...
            if not self._check_organization_access(requesting_user, org_id):
                if not hasattr(requesting_user, 'organization') or not getattr(requesting_user, 'organization', None):
                    return {"success": True, "groups": []}
                return {"success": False, "message": "Insufficient access to organization data"}
            groups = TrustGroupMembership.objects.filter(
            ...
            for membership in groups:
                ...
                formatted_groups.append({
                    ...
                    "member_count": group.memberships.count() if hasattr(group, 'memberships') else None,

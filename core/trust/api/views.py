...
from .permissions import TrustRelationshipPermission


def validate_trust_operation(operation_type: str, **kwargs) -> Dict[str, Any]:
    """
    Validate trust operations before executing them.
    
    Args:
        operation_type: Type of operation (create_relationship, approve_relationship, etc.)
    ...
    if operation_type == 'create_relationship':
        source_org = kwargs.get('source_org')
        target_org = kwargs.get('target_org')
        
        if source_org == target_org:
            return {'valid': False, 'message': 'Cannot create relationship with same organization'}
        ...
        if existing:
            return {'valid': False, 'message': 'Trust relationship already exists'}

...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        # Additional options for compatibility with tests
        parser.add_argument(
            '--format',
            choices=['text', 'json', 'csv'],
            help='Output format (alias for --output-format)',
        )
        parser.add_argument(
            '--output-file',
            help='Save report to file (alias for --save-to-file)',
        )
        parser.add_argument(
            '--check-expired',
            action='store_true',
            help='Check for expired relationships',
        )
        parser.add_argument(
            '--integrity-check',
            action='store_true',
            help='Perform data integrity check',
        )
        parser.add_argument(
            '--statistics',
            action='store_true',
        ...
        parser.add_argument(
            '--start-date',
            help='Start date for activity reports (YYYY-MM-DD)',
        )
        parser.add_argument(
            '--end-date',
            help='End date for activity reports (YYYY-MM-DD)',
        )
    
    def handle(self, *args, **options):
        """Handle the command execution."""
        # Set default values for options that might not be provided
        options.setdefault('report_type', 'summary')
        options.setdefault('days', 30)
        options.setdefault('output_format', 'text')
        options.setdefault('save_to_file', None)
        options.setdefault('organization', None)
        options.setdefault('format', None)
        options.setdefault('output_file', None)
        options.setdefault('check_expired', False)
        options.setdefault('integrity_check', False)
        options.setdefault('statistics', False)
        options.setdefault('start_date', None)
        options.setdefault('end_date', None)
        
        # Handle alias options
        if options.get('format') and not options.get('output_format'):
            options['output_format'] = options['format']
        if options.get('output_file') and not options.get('save_to_file'):
            options['save_to_file'] = options['output_file']
        
        report_data = {}
        
        # Handle special report types
        if options.get('check_expired'):
            report_data['expired'] = self.check_expired_relationships()
        
        if options.get('integrity_check'):
            report_data['integrity'] = self.check_data_integrity()
        
        if options.get('statistics'):
            report_data['statistics'] = self.generate_statistics()
    ...
    def check_expired_relationships(self):
        """Check for expired trust relationships."""
        now = timezone.now()
        expired = TrustRelationship.objects.filter(
            valid_until__lt=now,
            status='active'
        )
            ...
            'relationships': [
                {
                    ...
                    'source': rel.source_organization,
                    'target': rel.target_organization,
                    'expired_date': rel.valid_until,
                    'trust_level': rel.trust_level.name
                }
                for rel in expired[:10]  # Limit to first 10
            ]
        }
    ...
    def check_data_integrity(self):
        ...
        # Check for relationships without trust levels
        no_trust_level = TrustRelationship.objects.filter(trust_level__isnull=True).count()
        if no_trust_level > 0:
            issues.append(f"{no_trust_level} relationships missing trust level")
        
        # Check for self-referencing relationships
        self_ref = TrustRelationship.objects.filter(
            source_organization=F('target_organization')
        ).count()
        if self_ref > 0:
            issues.append(f"{self_ref} self-referencing relationships found")
        
        # Check for orphaned group memberships
        orphaned_memberships = TrustGroupMembership.objects.filter(
            trust_group__isnull=True
        ).count()
        if orphaned_memberships > 0:
            issues.append(f"{orphaned_memberships} orphaned group memberships")
    ...
    def generate_statistics(self):
        ...
        return {
            ...
            'total_groups': TrustGroup.objects.count(),
            'total_memberships': TrustGroupMembership.objects.count(),
            'trust_level_distribution': dict(
                TrustRelationship.objects.values('trust_level__name')
                .annotate(count=Count('id'))
                .values_list('trust_level__name', 'count')
            )
        }
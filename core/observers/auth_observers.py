...
class SecurityAlertObserver(AuthenticationObserver):
    ...
    def update(self, event_type: str, user: CustomUser, event_data: Dict) -> None:
        """Alias for notify method for test compatibility"""
        self.notify(event_type, user, event_data)
...
class ConsoleLoggingObserver(AuthenticationObserver):
    ...
    def notify(self, event_type: str, user: CustomUser, event_data: Dict) -> None:
        if event_type == 'login_success':
            print(f"LOGIN SUCCESS for user {getattr(user, 'username', None)} from {event_data.get('ip_address', 'unknown')}")
        elif event_type == 'login_failed':
            print(f"LOGIN FAILED for user {getattr(user, 'username', None)} - {event_data.get('failure_reason', 'Unknown reason')}")
        elif event_type == 'password_changed':
            print(f"PASSWORD CHANGED for user {getattr(user, 'username', None)}")
        else:

    ...
    <div className="App">
      ...
      <main className="main-content">
        <div className="container">
          {/* Dashboard */}
          <Dashboard active={activePage === 'dashboard'} user={user} userRole={userRole} userOrganization={userOrganization} />
          ...
          <ThreatDashboard 
            ...
            onNavigate={(pageId) => showPage(pageId)}
          />
...
// Dashboard Component
function Dashboard({ active, user, userRole, userOrganization }) {
  // D3 Chart References
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const dashboardRes = await fetch('http://127.0.0.1:8000/api/v1/dashboard/overview/', { headers });
      if (dashboardRes.ok) {
        const dashboardData = await dashboardRes.json();
        setDashboardStats(dashboardData.stats || {});
      }

      // Fetch indicator statistics
      const indicatorStatsRes = await fetch('http://127.0.0.1:8000/api/indicators/stats/', { headers });
      if (indicatorStatsRes.ok) {
        const indicatorStatsData = await indicatorStatsRes.json();
        setIndicatorStats(indicatorStatsData);
      }

      // Fetch recent indicators
      const recentRes = await fetch('http://127.0.0.1:8000/api/indicators/recent/', { headers });
      if (recentRes.ok) {
        const recentData = await recentRes.json();
        setRecentIndicators(recentData.indicators || []);
      }

      // Fetch organizations
      const orgsRes = await fetch('http://127.0.0.1:8000/api/v1/organizations/list_organizations/', { headers });
      if (orgsRes.ok) {
        const orgsData = await orgsRes.json();
        setOrganizations(orgsData.data?.organizations || []);
      }

      // Fetch threat feeds (using general API)
      try {
        const feedsRes = await fetch('http://127.0.0.1:8000/api/v1/threat-feeds/external/', { headers });
        if (feedsRes.ok) {
          const feedsData = await feedsRes.json();
          setThreatFeeds(feedsData.feeds || []);
        }
      } catch (feedError) {
        console.warn('Could not fetch threat feeds:', feedError);
      }
  ...
  // Fetch data when component mounts or becomes active
  useEffect(() => {
    if (active) {
      fetchDashboardData();
    }
  ...
  useEffect(() => {
    if (active && chartRef.current && !loading) {
  ...
  const createThreatActivityChart = () => {
    ...
    // Generate data based on actual indicator stats
    // Create trend data based on recent indicators
    const generateTrendData = () => {
      const days = 30;
      const data = [];
      const today = new Date();
      const totalIndicators = indicatorStats.total || 13928;
      const avgPerDay = Math.floor(totalIndicators / 365); // Average per day over a year
    ...
    <section id="dashboard" className={`page-section ${active ? 'active' : ''}`}>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="stat-value">{loading ? '...' : (indicatorStats.total || 0).toLocaleString()}</div>
          <div className="stat-change increase">
            <span><i className="fas fa-arrow-up"></i></span>
            <span>{indicatorStats.recent_24h || 0} in last 24h</span>
          </div>
        </div>
        <div className="stat-card">
          <div className="stat-title">
          ...
          <div className="stat-change increase">
            <span><i className="fas fa-arrow-up"></i></span>
            <span>{organizations.filter(org => org.is_active).length} active</span>
          </div>
        </div>
        <div className="stat-card">
          <div className="stat-title">
          ...
          <div className="stat-value">{loading ? '...' : (dashboardStats.total_feeds || threatFeeds.length || 0)}</div>
          <div className="stat-change increase">
            <span><i className="fas fa-arrow-up"></i></span>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value">{loading ? '...' : (dashboardStats.active_users || 0)}</div>
          <div className="stat-change increase">
            <span><i className="fas fa-arrow-up"></i></span>
            ...
            <div className="card-content">
              {loading ? (
                <div style={{ textAlign: 'center', padding: '2rem' }}>
                  <div style={{ fontSize: '1rem', color: '#666' }}>Loading recent indicators...</div>
                </div>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {recentIndicators.slice(0, 10).map((indicator, index) => {
                      const getConfidenceBadge = (confidence) => {
                        if (confidence >= 80) return 'badge-high';
                        if (confidence >= 50) return 'badge-medium';
                        return 'badge-low';
                      };

                      const formatIndicatorValue = (value, type) => {
                        if (type === 'file_hash' && value.length > 20) {
                      ...
                      const getTypeIcon = (type) => {
                        switch (type) {
                          case 'ip': return 'fas fa-server';
                          case 'domain': return 'fas fa-globe';
                          case 'url': return 'fas fa-link';
                          case 'file_hash': return 'fas fa-file-code';
                      ...
                      return (
                        <tr key={indicator.id || index}>
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                              <i className={getTypeIcon(indicator.type)} style={{ color: '#666' }}></i>
                              {indicator.type.toUpperCase()}
                            </div>
                          </td>
                          <td title={indicator.value}>
                            <code style={{ fontSize: '0.85rem', backgroundColor: '#f8f9fa', padding: '2px 4px', borderRadius: '3px' }}>
                              {formatIndicatorValue(indicator.value, indicator.type)}
                            </code>
                          </td>
                          <td>{indicator.threat_feed ? `Feed ${indicator.threat_feed}` : 'Internal'}</td>
                          <td>
                ...
                <div style={{ textAlign: 'center', padding: '2rem', color: '#666' }}>
                  No recent indicators available
                </div>
              )}
            </div>
          </div>
        ...
        <div className="side-panels">
          ...
          <div className="card">
            ...
            <div className="card-content">
              {loading ? (
                <div style={{ textAlign: 'center', padding: '2rem' }}>
                  <div style={{ fontSize: '1rem', color: '#666' }}>Loading institutions...</div>
                </div>
                  ...
                  {organizations.slice(0, 6).map((org, index) => {
                    ...
                    const getTypeColor = (type) => {
                      switch (type?.toLowerCase()) {
                        case 'government': return '#28a745';
                        case 'university': case 'education': return '#007bff';
                        case 'healthcare': return '#dc3545';
                        case 'financial': return '#ffc107';
                        case 'technology': return '#6f42c1';
                        default: return '#6c757d';
                      }
                    };

                    const formatLastActivity = (createdDate) => {
                      try {
                    ...
                    const trustLevel = org.is_active ? (org.is_verified ? 90 : 60) : 30;

                    return (
                      <li key={org.id} className="institution-item">
                        <div 
                          className="institution-logo" 
                          style={{ backgroundColor: getTypeColor(org.organization_type) }}
                        >
                          {getLogoInitials(org.name)}
                        </div>
                        <div className="institution-details">
                          <div className="institution-name" title={org.name}>
                            {org.name.length > 25 ? org.name.substring(0, 25) + '...' : org.name}
                          </div>
                          <div className="institution-stats">
                            <div className="stat-item">
                              <i className="fas fa-users"></i> {org.user_count || 0} users
                            </div>
                            <div className="stat-item">
                              <i className="fas fa-clock"></i> {formatLastActivity(org.created_date)}
                            </div>
                          </div>
                          <div style={{ fontSize: '0.75rem', color: '#666', marginTop: '2px' }}>
                            {org.organization_type?.charAt(0).toUpperCase() + org.organization_type?.slice(1) || 'Unknown'}
                          </div>
                        </div>
                        <div className="trust-level" title={`Trust Level: ${trustLevel}%`}>
                          <div 

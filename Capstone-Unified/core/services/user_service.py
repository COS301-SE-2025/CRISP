...
class UserService:
    ...
    def change_username(self, user_id, new_username, changed_by):
        ...
        # Check permissions
        if changed_by.role not in ['BlueVisionAdmin', 'publisher']:
            # Allow users to change their own username
            if changed_by.id != user.id:
                return {
                    'success': False,
                    'message': 'Insufficient permissions to change username'
                }
        
        # Additional permission check for non-BlueVisionAdmin users
        if changed_by.role != 'BlueVisionAdmin' and changed_by.id != user.id:
            if not changed_by.organization or changed_by.organization != user.organization:
                return {
                    'success': False,
                    'message': 'Can only change username for users in your own organization'
                }
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"Username changed from '{old_username}' to '{new_username}' by {changed_by.username}")
            ...
            return {
                'success': False,
                'message': f"Failed to change username: {str(e)}"
            }
  ...
  if (contentLength !== '0' && contentType && contentType.includes('application/json')) {
    try {
      ...
      data = { 
        ...
        raw_response: response.statusText 
      };
    }
  }
...
export const testGmailConnection = async () => {
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = errorData.message || `Connection test failed with status ${response.status}`;
    ...
    return {
      ...
      service_type: 'gmail_smtp'
    };
  } catch (error) {
    console.error('Gmail connection test failed:', error);
    throw new Error(error.message || 'Gmail connection test failed - please check your email configuration');
  }
};
...
export const sendTestEmail = async (email) => {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 500) {
        throw new Error(errorData.message || 'Email service temporarily unavailable');
      }
    ...
    // Enhanced result with timestamp
    return {
      ...result,
      timestamp: new Date().toISOString(),
...
export const getUserDetails = async (userId) => {
  const response = await fetch(`${API_URL}users/${userId}/get_user/`, {
    method: 'GET',
    headers: { ...authHeader() }
  });
...
export const updateUser = async (userId, userData) => {
  const response = await fetch(`${API_URL}users/${userId}/update_user/`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', ...authHeader() },
    body: JSON.stringify(userData)
  });
...
export const deactivateUser = async (userId, reason = '') => {
  const response = await fetch(`${API_URL}users/${userId}/deactivate_user/`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeader() },
    body: JSON.stringify({ reason })
  });
...
export const reactivateUser = async (userId, reason = '') => {
  const response = await fetch(`${API_URL}users/${userId}/reactivate_user/`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeader() },
    body: JSON.stringify({ reason })
  });
...
export const getNotificationPreferences = async () => {
  try {
    const response = await fetch(`${API_URL}alerts/preferences/`, {
      method: 'GET',
      headers: { ...authHeader() }
    });
...
export const updateNotificationPreferences = async (preferences) => {
  try {
    const response = await fetch(`${API_URL}alerts/preferences/update/`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeader() },
...
// Generic API functions for enhanced components
